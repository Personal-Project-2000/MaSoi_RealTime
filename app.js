...
io.sockets.on("connection", (socket) => {
    ...
    socket.on("protected", (data) => {
        ...
        if (info.user !== "") {
            ...
            timeList[indexRoomOfTimeList].story += randomStoryGuard(false, guardInfo.name, userProtected.name)
        } else {
    ...
    function call(roomId, number) {
        ...
        if (number < 11) {
            ...
            if (playerVote.length > 0) {
                timeList[indexRoom1].story += randomStoryVote(true, userListStory)
            } else {
